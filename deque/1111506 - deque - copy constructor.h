...
public:
      ...
      : myData()
   {
      if( right.myData.mySize > 0 )
      {
         ...
         for (size_type i = 0; n>=0; i++) {
             for (size_type j = 0; j + tmpj< dequeSize; j++) {
                 tmp[n-1]=right.myData.map[tmpi][tmpj];
                 n--;
             }
             tmpj = 0;
         }
         n = right.myData.mySize;*/

         for (size_type i = 0; i < myData.mapSize; i++)
             myData.map[i] = nullptr;
         for (size_type i = 0;i<right.myData.mapSize && n>0; i++) {
             myData.map[i] = new value_type[dequeSize];
             for (size_type j = 0; j < dequeSize && n > 0; j++,tmpj++) {
                 if (tmpj == dequeSize) {
                     tmpj = 0;
                     tmpi++;
