   ...
   iterator erase( const_iterator where )
   {
      ...
      else
      {
         ...
         if( off < myData.mySize / 2 ) // closer to front
         {
             ...
             i = (off + myData.myOff) / dequeSize;

             j = (myData.myOff + off) % dequeSize;
             ...
             //if (myData.map[i] != 0)
                 //if (j == dequeSize - 1 && myData.map[i][j-1] == 0)
                     //myData.map[i] = 0;
         ...
         else // closer to back
         {
             ...
             int i = 0;
             int j = 0;
             int n = myData.mySize - off - 1;
             i = (myData.myOff + off + 1) / dequeSize;
             j = (myData.myOff + off + 1) % dequeSize;
             int i2 = i;
             int j2 = j - 1;
             while (n > 0) {
